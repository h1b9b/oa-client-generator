...
export function createIndexSignature(
  type: ts.TypeNode,
  {
    ...
    indexName?: string;
    indexType?: ts.TypeNode | ts.LiteralTypeNode;
    decorators?: Array<ts.Decorator>;
    modifiers?: Array<ts.Modifier>;
  } = {}
) {
...
export function changePropertyValue(
  ...
  const property = o.properties.find((p) => ts.isPropertyAssignment(p) && getName(p.name) === name);
  if (property && ts.isPropertyAssignment(property)) {
    ts.factory.updatePropertyAssignment(property, property.name, value);
  } else {
